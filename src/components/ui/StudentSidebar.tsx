...
import {
  LayoutDashboard, Calendar, BookOpen, FileText, DollarSign, ChevronRight, Menu
} from "lucide-react";
...
const navGroups = [
  {
    group: "HỌC TẬP",
    items: [
      { label: "Tổng quan", icon: <LayoutDashboard size={18} />, href: "/student/dashboard" },
      { label: "Lịch học", icon: <Calendar size={18} />, href: "/student/calendar" },
      { label: "Lớp học của tôi", icon: <BookOpen size={18} />, href: "/student/classes" },
      { label: "Bài tập", icon: <FileText size={18} />, href: "/student/homework" },
    ],
  },
  {
    group: "TÀI CHÍNH",
    items: [
      { label: "Học phí", icon: <DollarSign size={18} />, href: "/student/finance" },
    ],
  },
];

export default function StudentSidebar({ collapsed }: { collapsed?: boolean }) {
    ...
    >
      ...
      <div className="h-[64px] flex items-center px-5 border-b border-border shrink-0">
        <div className="flex items-center gap-3">
          ...
          {!collapsed && (
            <span className="font-bold text-[15px] text-text-main truncate">Center Brain Student</span>
          )}
        </div>
      </div>
